...
from torch.nn.functional import _mha_shape_check, _in_projection_packed, _in_projection, pad, scaled_dot_product_attention, linear
import torch
import warnings
...
from torch.nn import Dropout, Linear, LayerNorm, MultiheadAttention
...
def multi_head_attention_forward(
    ...
    attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len * bsz, int(head_dim * num_heads))
...
class MultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dim_head=64, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.kdim = int(self.head_dim * num_heads)
        self.vdim = int(self.head_dim * num_heads)

        # assert self.head_dim * num_heads == self.embed_dim, "embed_dim must be divisible by num_heads"

        if self._qkv_same_embed_dim is False:
            # self.q_proj_weight = Parameter(torch.empty((embed_dim, embed_dim), **factory_kwargs))
            self.q_proj_weight = Parameter(torch.empty((embed_dim, self.kdim), **factory_kwargs))
            self.k_proj_weight = Parameter(torch.empty((embed_dim, self.kdim), **factory_kwargs))
            self.v_proj_weight = Parameter(torch.empty((embed_dim, self.vdim), **factory_kwargs))
            self.register_parameter('in_proj_weight', None)
        else:
            # self.in_proj_weight = Parameter(torch.empty((3 * embed_dim, embed_dim), **factory_kwargs))
            self.in_proj_weight = Parameter(torch.empty((3 * self.kdim, embed_dim), **factory_kwargs))
        ...
        if bias:
            ...
            self.in_proj_bias = Parameter(torch.empty(3 * self.kdim, **factory_kwargs))
        else:
        ...
        if add_bias_kv:
            ...
            self.bias_v = Parameter(torch.empty((1, 1, self.kdim), **factory_kwargs))
        else:
...
class CustomTransformerEncoderLayer(TransformerEncoderLayer):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int = 2048, dim_head: int = 64, 
                 dropout: float = 0.1, activation: Union[str, Callable[[Tensor], Tensor]] = F.relu,
                 layer_norm_eps: float = 1e-5, batch_first: bool = False, norm_first: bool = False,
                 device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(TransformerEncoderLayer, self).__init__()
        self.self_attn = MultiheadAttention(d_model, nhead, dim_head=dim_head, dropout=dropout, 
