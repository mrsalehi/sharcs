...
class BranchyRoberta(RobertaForSequenceClassification):
    def __init__(self, config):
        ...
        self.internal_classifier_thresh = config.internal_classifier_thresh  # here the threshold is the threshold on entropy
...
class BranchyBert(BertForSequenceClassification):
    def __init__(self, config):
        config.output_hidden_states = True
        super().__init__(config)
        assert getattr(config, "internal_classifier_layer", None) is not None
...
class BranchyDistilBert(DistilBertForSequenceClassification):
    def __init__(self, config):
        config.output_hidden_states = True
        super().__init__(config)
        assert getattr(config, "internal_classifier_layer", None) is not None
