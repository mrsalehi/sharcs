...
class PABEERoberta(RobertaForSequenceClassification):
    def __init__(self, config):
        ...
        assert config.internal_classifier_all_layers, "Only all layers is supported for PABEE"
            ...
            if self.num_labels == 1:
                ...
                if not self.training:
                    ...
                    if self.config.internal_classifier_all_layers:
                        ...
                        sample_patience = 0
                        for i in range(len(logits_internal)):
                            pred = logits_internal[i].argmax(dim=-1)

                            # NOTE: patience can be anything between 1 and the internal_classifier_layer (including both)
                            if len(preds) > 0:
                                if pred == preds[-1]:
                                    sample_patience += 1
                                else:
                            ...
                            if sample_patience == self.patience:
...
class PABEEBert(BertForSequenceClassification):
    def __init__(self, config):
        config.output_hidden_states = True
        super().__init__(config)
        assert getattr(config, "internal_classifier_layer", None) is not None
...
class PABEEDistilBert(DistilBertForSequenceClassification):
    def __init__(self, config):
        config.output_hidden_states = True
        super().__init__(config)
        assert getattr(config, "internal_classifier_layer", None) is not None
